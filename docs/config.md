...
| `armed` | `false` | whether dictation was left switched on. `/voice` writes it, so a new window opens the way you left the last one |
...
`grep sudo /etc/group` is a search and `sudo apt update` is not. It is off
everywhere unless you turn it on; `ah remote serve --sudo` is how you say yes
for sessions driven from a phone, see [remote](remote.md).
